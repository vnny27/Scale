...
__global__ void v5_warp_tiled_matmul(
    ...
    int local_row = warp_m * WARP_TILE_M + lane_m * THREAD_TILE_M;
    int local_col = warp_n * WARP_TILE_N + lane_n * THREAD_TILE_N;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; ++i) {
        v5_warp_tiled_matmul<<<dimGrid, dimBlock>>>(A_dev, B_dev, C_dev);
    }
