...
__global__ void tiled_matmul(
    ...
    for (int tile_k = 0; tile_k < MATRIX_K; tile_k += BLOCK_K) {
        ...
        for (int load_i = 0; load_i < A_LOADS_PER_THREAD; ++load_i) {
            ...
            int a_k = a_idx / BLOCK_M;
            int a_row = a_idx % BLOCK_M;

            sharedA[a_k][a_row] =
                A[(tile_k + a_k) * MATRIX_M + blockIdx.y * BLOCK_M + a_row];
        }

        #pragma unroll
        for (int load_i = 0; load_i < B_LOADS_PER_THREAD; ++load_i) {
            int b_idx = tid + load_i * BLOCK_THREADS;
            int b_k = b_idx % BLOCK_K;
            int b_col = b_idx / BLOCK_K;

            sharedB[b_k][b_col] =
                B[(blockIdx.x * BLOCK_N + b_col) * MATRIX_K + tile_k + b_k];
        }
        ...
        for (int k = 0; k < BLOCK_K; ++k) {
            c_val = fmaf(sharedA[k][local_row], sharedB[k][local_col], c_val);
        }

        __syncthreads();
    }

    C[global_col * MATRIX_M + global_row] = c_val;
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; ++i) {
        tiled_matmul<<<dimGrid, dimBlock>>>(A_dev, B_dev, C_dev);
    }
    ...
    if (verify && launchErr == cudaSuccess && syncErr == cudaSuccess) {
        ...
        if (validate(C_answer, C_host, C_size)) {
            std::cout << ">>> Tiled kernel test pass!" << std::endl;
        } else {
            std::cout << ">>> Tiled kernel test fail!" << std::endl;
        ...
        if (compare_cublas) {
            if (cublasOk) {
                ...
                std::cout << ">>> cuBLAS verification skipped because SGEMM did not complete successfully."
                          << std::endl;
            }
        }
