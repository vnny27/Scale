...
__global__ void micro4_matmul(
    ...
    int tid = threadIdx.x;
    int thread_m = tid % THREADS_M;
    int thread_n = tid / THREADS_M;
    ...
    for (int j = 0; j < THREAD_TILE_N; ++j) {
        #pragma unroll
        for (int i = 0; i < THREAD_TILE_M; ++i) {
            C[(global_col + j) * MATRIX_M + global_row + i] = c_val[i][j];
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; ++i) {
        micro4_matmul<<<dimGrid, dimBlock>>>(A_dev, B_dev, C_dev);
    }
