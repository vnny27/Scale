...
__global__ void base_matmul(
    ...
    for (int k = 0; k < MATRIX_K; ++k) {
        sum = fmaf(A[row + k * MATRIX_M], B[k + col * MATRIX_K], sum);
    }

    C[row + col * MATRIX_M] = sum;
}
...
int main(int argc, char* argv[]) {
    ...
    dim3 dimGrid((MATRIX_N + BLOCK_DIM_X - 1) / BLOCK_DIM_X,
                 (MATRIX_M + BLOCK_DIM_Y - 1) / BLOCK_DIM_Y,
                 1);
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; ++i) {
        base_matmul<<<dimGrid, dimBlock>>>(A_dev, B_dev, C_dev);
    }
    ...
    if (verify && launchErr == cudaSuccess && syncErr == cudaSuccess) {
        ...
        if (validate(C_answer, C_host, C_size)) {
            std::cout << ">>> Base kernel test pass!" << std::endl;
        } else {
            std::cout << ">>> Base kernel test fail!" << std::endl;
