...
__global__ void matmul(
    ...
    for (int k = 0; k < MATRIX_K; k += BLOCK_K) {
        for (int idx = threadIdx.x; idx < BLOCK_M * BLOCK_K; idx += BLOCK_THREADS) {
            int m = idx % BLOCK_M;
            int kk = idx / BLOCK_M;
            shared_a[kk][m] = A[(block_base_row + m) + (k + kk) * MATRIX_M];
        }

        for (int idx = threadIdx.x; idx < BLOCK_K * BLOCK_N; idx += BLOCK_THREADS) {
            int kk = idx % BLOCK_K;
            int n = idx / BLOCK_K;
            shared_b[n][kk] = B[(k + kk) + (block_base_col + n) * MATRIX_K];
        }
        ...
        for (int kk = 0; kk < BLOCK_K; kk += MMA_K) {
            ...
            a_frag[0] = f32_to_tf32(shared_a[a_col0][a_row0]);
            a_frag[1] = f32_to_tf32(shared_a[a_col0][a_row1]);
            a_frag[2] = f32_to_tf32(shared_a[a_col1][a_row0]);
            a_frag[3] = f32_to_tf32(shared_a[a_col1][a_row1]);
            ...
            b_left[0] = f32_to_tf32(shared_b[b_col_left][b_row0]);
            b_left[1] = f32_to_tf32(shared_b[b_col_left][b_row1]);
            b_right[0] = f32_to_tf32(shared_b[b_col_right][b_row0]);
            b_right[1] = f32_to_tf32(shared_b[b_col_right][b_row1]);
