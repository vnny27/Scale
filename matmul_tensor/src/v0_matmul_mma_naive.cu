...
__global__ void matmul(
    ...
    for (int k = 0; k < MATRIX_K; k += MMA_K) {
        ...
        int a_col0 = thread_in_group;
        int a_col1 = thread_in_group + 4;
        a_frag[0] = f32_to_tf32(A[(base_row + a_row0) + (k + a_col0) * MATRIX_M]);
        a_frag[1] = f32_to_tf32(A[(base_row + a_row1) + (k + a_col0) * MATRIX_M]);
...
int main(int argc, char* argv[]) {
    ...
    int total_tiles = (MATRIX_M / MMA_M) * (MATRIX_N / MMA_N);
    dim3 dimGrid((total_tiles + WARPS_PER_BLOCK - 1) / WARPS_PER_BLOCK, 1, 1);
